...
if (isset($_GET['search']) && !empty($_GET['search'])) {
    ...
    $searchQuery = "WHERE name LIKE '%$searchTerm%' OR class LIKE '%$searchTerm%' OR phonenumber LIKE '%$searchTerm%' OR parentname LIKE '%$searchTerm%'";
}

$result = $conn->query("SELECT * FROM students $searchQuery ORDER BY id ASC");
?>
    ...
    <div class="max-w-7xl mx-auto p-6">
        ...
        <form method="GET" class="flex space-x-2 mb-6">
            <input 
                type="text" 
                name="search" 
                placeholder="Search by name, class, phone number, or parent's name" 
            ...
            <table class="table-auto w-full bg-white shadow-md rounded-md border-collapse">
                <thead>
                    <tr class="bg-gray-200 text-gray-700">
                        ...
                        <th class="border px-4 py-2">Parent's Name</th>
                ...
                <tbody>
                    <?php while ($row = $result->fetch_assoc()): ?>
                        <tr class="hover:bg-gray-100">
                            ...
                            <td class="border px-4 py-2"><?php echo htmlspecialchars($row['class']); ?></td>
                            <td class="border px-4 py-2"><?php echo htmlspecialchars($row['phonenumber']); ?></td>
                            <td class="border px-4 py-2"><?php echo htmlspecialchars($row['parentname']); ?></td>
                            <td class="border px-4 py-2 text-center">
                                <a href="edit_student.php?id=<?php echo $row['id']; ?>" class="text-blue-500 hover:underline">Edit</a> |
                                <a href="?delete_id=<?php echo $row['id']; ?>" onclick="return confirm('Are you sure you want to delete this student?');" class="text-red-500 hover:underline">Delete</a>
                            </td>
                        </tr>
                    <?php endwhile; ?>
                </tbody>
        ...
        <?php else: ?>
            <p class="text-gray-700">No students found.</p>
        <?php endif; ?>
