...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $social_science = isset($_POST['social_science']) ? (int)$_POST['social_science'] : 0;
    $religion = isset($_POST['religion']) ? (int)$_POST['religion'] : 0;
    $ict = isset($_POST['ict']) ? (int)$_POST['ict'] : 0;
    
    // Calculate total and average
    $total_marks = $bangla + $english + $math + $science + $social_science + $religion + $ict;
    ...
    if (empty($student_id) || empty($student_name) || empty($roll_number) || empty($exam_type) || empty($year)) {
        ...
        $check_query = "SELECT * FROM results WHERE student_id = '$student_id' AND exam_type = '$exam_type' AND year = '$year' AND class = 'class7'";
        ...
        if (mysqli_num_rows($check_result) > 0) {
            ...
            $update_query = "UPDATE results SET 
                ...
                average = $average,
                grade = '$grade'
                WHERE student_id = '$student_id' AND exam_type = '$exam_type' AND year = '$year' AND class = 'class7'";
            ...
            // Insert new result
            $insert_query = "INSERT INTO results (student_id, student_name, roll_number, class, exam_type, year, bangla, english, math, science, social_science, religion, ict, total_marks, average, grade) 
                VALUES ('$student_id', '$student_name', '$roll_number', 'class7', '$exam_type', '$year', $bangla, $english, $math, $science, $social_science, $religion, $ict, $total_marks, $average, '$grade')";
...
<head>
    ...
    <title>Upload Result - Class 7</title>
...
<body>
    <div class="container">
        <h2 class="mb-4">Upload Result - Class 7</h2>
            ...
            <div class="row">
                <div class="col-md-4">
                    <div class="form-group">
                        ...
                        <input type="number" class="form-control" id="science" name="science" min="0" max="100" required>
                    </div>
                </div>
                <div class="col-md-4">
                    <div class="form-group">
                        <label for="social_science">Social Science</label>
                        <input type="number" class="form-control" id="social_science" name="social_science" min="0" max="100" required>
                    </div>
                </div>
                <div class="col-md-4">
                    <div class="form-group">
                        <label for="religion">Religion</label>
                        <input type="number" class="form-control" id="religion" name="religion" min="0" max="100" required>
                    </div>
                </div>
            </div>
